...
export class CorpusClient {
  ...
  manifest(): Promise<Manifest> {
    if (!this.manifestPromise) {
      this.manifestPromise = fetch(`${this.baseUrl}manifest.json`).then((response) => {
        if (!response.ok) {
          throw new Error(
            'Search index not found. It is generated by CI — run `npm run harvest && npm run build:index` for a local copy.'
          )
        }
